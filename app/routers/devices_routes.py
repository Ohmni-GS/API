...
from app.schemas import DefaultResponse, Device, DeviceData, DeviceLatestData, Devices, HTTPErrorRequest
...
@devices_router.get("/{device_id}/connect", response_model=DefaultResponse, status_code=status.HTTP_200_OK, responses={408: {"description": "O dispositivo não respondeu no tempo estipulado.", "model": HTTPErrorRequest}, 500: {"description": "Erro ao se conectar no Broker MQTT", "model": HTTPErrorRequest}})
async def connect_device(device_id: str, device_service: DeviceService = Depends(DeviceService)) -> DefaultResponse:
